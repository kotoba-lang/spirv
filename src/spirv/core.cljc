(ns spirv.core
  "SPIR-V assembly as data — 'hiccup for the GPU IR'. A vector of EDN instructions compiles to SPIR-V
   ...
   composable data you fork and diff like a scene — a sibling to kotoba.wgsl on the GPU side. `.cljc`.

   SPIR-V is flat SSA (no infix expressions), so unlike kotoba.{scad,verilog,wgsl} this does NOT use the
   shared kotoba.expr core — each instruction is one line. NOTE: this is the assembly-as-data layer; a
